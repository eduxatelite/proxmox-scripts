...
ask_config() {
  ...
  if [[ ${#storages[@]} -eq 0 ]]; then
    warn "No se detectaron storages. Usando 'local-lvm' por defecto."
    storages=("local-lvm")
  fi
  ...
  while [[ ! "$VM_NAME" =~ ^[a-zA-Z0-9-]+$ ]]; do
    warn "Nombre inválido — solo letras, números y guiones (-)."
    read -rp "Nombre de la VM [${def_name}]: " VM_NAME
    VM_NAME="${VM_NAME:-$def_name}"
  done
...
download_rocky_image() {
  ...
  log "Imagen descargada → ${img_path}"
}
...
create_vm() {
  ...
  info "Importando imagen como disco..."
  ...
  [[ -z "$disk_val" ]] && error "No se encontró el disco importado"

  qm set "$VMID" --scsi0 "${disk_val},discard=on" || error "Fallo al asignar disco"
  qm set "$VMID" --ide2 "${STORAGE}:cloudinit"
  qm set "$VMID" --boot "order=scsi0"

  info "Redimensionando disco a ${DISK_SIZE}GB..."
  qm resize "$VMID" scsi0 "${DISK_SIZE}G" || warn "No se pudo redimensionar el disco"
...
# =============================================================================
post_install() {
  info "Configuración aplicada vía Cloud-Init (SELinux, timezone, teclado, dnf update)"
